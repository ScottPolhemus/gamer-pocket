...
export const PlayerProvider = ({
  ...
  useEffect(() => {
    if (!initialized) {
      playerRef.current = new GameBoyPlayer(screenCanvasRef.current, {
        mediaStreamWorkerSrc: `/static/media-stream-worker.js`
      })
      setInitialized(true)
    }
  }, [])
  ...
  useEffect(() => {
    if (currentGame && !loadedGame && freeze && freezeScreen && !playing && !paused) {
      setPaused(true)
    }
...
export const usePlayer = () => {
  ...
  const openROM = (file) => {
    return playerRef.current.openROM(file)
      .then(() => {
  ...
  const restart = () => {
    ...
    return playerRef.current.loadROM(loadedGame || currentGame)
      .then(openROM)
      .then(start)
  }
  ...
  const getSRAM = () => playerRef.current.loadSRAM(currentGame)
